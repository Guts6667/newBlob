...
export default function WorkCarousel({previousWork, nextWork}) {
  return (
    ...
    <div className="grid  md:grid-cols-2 gap-50 ">
      {previousWork && (
        <SelectedWorksCard img={previousWork.mainImage} client={previousWork.client} type={previousWork.type} path={"/works/" + previousWork.id} customClass={"h-[300px]"} />
      )}
      {nextWork && (
        <SelectedWorksCard img={nextWork.mainImage} client={nextWork.client} type={nextWork.type} path={"/works/" + nextWork.id} customClass={"h-[300px]"} />
      )}
    </div>
